...
HAS_GUARD = True
HAS_WOLF_KING = True
HUNTER_NIGHT_SHOOT = False
WITCH_DOUBLE_USE = False
NIGHT_BADGE_BREAKS = True
ALLOW_WOLF_SELF_BOMB = True
ELECTION_BEFORE_N1 = False
...
<head>
    ...
    <style>
        * {
        ...
        .panel-header.god {
            color: #7209b7;
            background: linear-gradient(135deg, #e5ccff 0%, #d4a5ff 100%);
            border-left: 4px solid #7209b7;
        }
        .panel-header.day {
            color: #d97706;
            background: linear-gradient(135deg, #ffd699 0%, #ffbb66 100%);
...
<body class="night">
    <div class="container">
        ...
        <div class="controls">
            ...
            <button onclick="testULSUnderstanding()" id="btn-test-uls" style="background: #9C27B0; color: white;">🧪 测试ULS++理解</button>
        </div>
        <!-- API配置模态窗口 -->
    ...
    <script>
        ...
        async function testULSUnderstanding() {
            ...
            statsOutput.innerHTML = '<div style="text-align: center; padding: 20px;"><div style="color: #9C27B0; font-size: 18px; margin-bottom: 10px;">🧪 测试ULS++理解能力</div><div style="color: #666;">正在测试LLM是否能理解ULS++编码...</div></div>';
            try {
                ...
                if (data.status === 'ok') {
                    statsOutput.innerHTML = `
                        <div style="padding: 20px;">
                            <div style="color: #4CAF50; font-size: 18px; margin-bottom: 15px;">✅ 测试成功！</div>
                            <div style="background: #f5f5f5; padding: 15px; border-radius: 8px; margin-bottom: 10px;">
                                <div style="font-weight: bold; margin-bottom: 10px;">LLM回答：</div>
                                <div style="white-space: pre-wrap; color: #333;">${data.answer}</div>
                            </div>
                            <div style="background: #e8f5e9; padding: 15px; border-radius: 8px;">
                                <div style="font-weight: bold; margin-bottom: 10px; color: #4CAF50;">✓ 正确答案：</div>
                                <div style="color: #333;">
                                    1. ${data.correct_answers.q1}<br>
                                    2. ${data.correct_answers.q2}<br>
                                    3. ${data.correct_answers.q3}
                                </div>
                            </div>
                        </div>
                    `;
                } else {
                    statsOutput.innerHTML = `<div style="color: #f44336; text-align: center; padding: 20px;">❌ 测试失败: ${data.message || 'Unknown error'}</div>`;
                }
            } catch (error) {
                statsOutput.innerHTML = `<div style="color: #f44336; text-align: center; padding: 20px;">❌ 错误: ${error.message}</div>`;
            }
        }
...
def call_llm(prompt, player_id):
    ...
    try:
        ...
        try:
            ...
            print(f"[LLM] API 调用失败: {api_error}，使用测试回复")
            # 使用测试回复
            test_responses = {
                0: "我觉得应该刀掉Player 5，他看起来最可疑。我支持狼人阵营。OUTPUT: 建议刀Player 5，我觉得他是好人。END",
                1: "Player 6好像很强，我们需要消除威胁。OUTPUT: 我们应该集中火力。END",
                2: "同意击杀Player 5，这是最优策略。OUTPUT: 赞成刀Player 5。END",
                3: "作为狼王，我提议击杀最强的玩家。OUTPUT: 我同意这个计划。END",
            }
            full_response = test_responses.get(player_id, f"OUTPUT: Player {player_id}的测试发言 END")
        ...
        print(f"\n[ERROR] Player {player_id} 调用失败:")
        print(f"  错误类型: {type(e).__name__}")
        print(f"  错误信息: {e}\n")
        return f"测试回复：Player {player_id}的临时发言"
...
@app.route('/api/test_uls_understanding', methods=['POST', 'OPTIONS'])
...
请简短回答（1-2句）。"""
        llm_response = requests.post(
            f"{LLM_CONFIG['npc_api_base']}/v1/chat/completions",
            headers={"Content-Type": "application/json"},
            json={
                "model": LLM_CONFIG['npc_model'],
        ...
        if llm_response.status_code == 200:
            ...
            print(f"[ULS_TEST] LLM回答: {answer}")
            response_data = {
                "status": "ok",
                "test_name": "ULS++ Understanding Test",
                "prompt": test_prompt,
                "answer": answer,
                "correct_answers": {
                    "q1": "3个玩家(P0, P1, P4)投票给Player 3",
            ...
            print(f"[ULS_TEST] LLM API error: {llm_response.status_code}")
            response = make_response(jsonify({"status": "error", "message": f"LLM API返回错误: {llm_response.status_code}"}), 500)
            response.headers['Access-Control-Allow-Origin'] = '*'
            return response
    except Exception as e:
        print(f"[ULS_TEST] Error: {e}")
        import traceback
        traceback.print_exc()
