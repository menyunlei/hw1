...
def test_llm_simple():
    ...
    try:
        ...
        # 简单的测试prompt
        simple_prompt = "请用OUTPUT和END标记回答：1+1等于几?\n\nOUTPUT: "
        ...
        error_msg = f"超时: API服务在10秒内未响应"
        print(f"[TEST_LLM] ✗ {error_msg}")
        return {"status": "✗ 失败", "error": error_msg, "api_base": api_base or "未获取"}, 504
    
    except requests.exceptions.ConnectionError as e:
        error_msg = f"无法连接: {str(e)[:200]}"
        print(f"[TEST_LLM] ✗ {error_msg}")
        return {"status": "✗ 失败", "error": error_msg, "api_base": api_base or "未获取"}, 503
