...
PLAYERS = [
    ...
    {"id": 7, "role": "Ԥ�Լ��Լ�", "emoji": "???", "color": "#2196F3", "alive": True},
...
def generate_difficulty_module_plan():
    ...
    if "A_双预对跳" in config.get("modules", []):
        ...
        real_seer = next((p for p in PLAYERS if p["role"] == "预言家"), None)

        fake_seer = random.choice(wolves) if wolves else None

        golden_candidates = [p for p in good_players if p["role"] != "预言家"]

        golden_water = random.choice(golden_candidates) if golden_candidates else None

        wolf_target_candidates = [p for p in good_players if p["role"] != "预言家"]
    ...
    if "C_票型搅动" in config.get("modules", []):
        ...
        if planned_real_seer_id is None:
            ...
            seer = next((p for p in PLAYERS if p["role"] == "预言家"), None)
...
NO text. NO explanation. ONLY the header line."""
            ...
            if player['role'] == '预言家:
                ...
                instruction = f"你是真预言家。昨晚验了Player {checked_player}，他是{'狼人' if is_werewolf else '好人'}。必须说'我是预言家并报告验人�?

            elif player['role'] in ['狼人', '狼王']:

                fake_result = "狼人" if random.random() > 0.6 else "好人"

                instruction = f"你是狼人阵营，必须跳预言家（�?我是预言家）并编造验人，例如'昨晚验了Player {checked_player}，他是{fake_result}'。绝对不能说你是狼人�?

            else:

                fake_result = "狼人" if random.random() > 0.6 else "好人"

                instruction = f"你必须跳预言家（�?我是预言家）并编造验人，例如'昨晚验了Player {checked_player}，他是{fake_result}'�?
...
def test_evaluation():
    ...
    test_dialogues = [
        ...
        {"type": "dialogue", "player_id": 7, "content": "我认为Player 2是真预言家因为他的发言逻辑清晰", "round": 1, "phase": "day"},
        ...
        {"type": "dialogue", "player_id": 7, "content": "根据概率�?如果Player 2是真预言家那么Player 5是狼的概率是80%", "round": 2, "phase": "day"},
